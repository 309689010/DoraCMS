...
import { entities } from './server/lib/entities'
// 仅测试用 **************************************************************************
import { Post } from './server/lib/entities/post'
// 仅测试用 **************************************************************************
...
async function init() {
  ...
  const sql = await getRepository('post')
  ...
  const sql2 = await getRepository('post')
    .createQueryBuilder('t')
    .update()
    .set({
      text: 111
  ...
  const result = await getRepository('post').insert({
    text: 333
  })

  console.log(result)

  const test = await getRepository('post').query('select 1 from dual')
  console.log(test)

  // const a = await getRepository('post').save(post)
  // console.log(a)
  // const b = await getRepository('post').find()
  // console.log(b)
  // const c = getRepository('post')
  //   .createQueryBuilder('t')
  //   .getSql()
  // console.log(c)
...
async function connectionMySQL() {
  await createConnection({
    ...
    logging: settings.TYPEORM_logging,
    maxQueryExecutionTime: settings.TYPEORM_maxQueryExecutionTime,
    synchronize: settings.TYPEORM_synchronize
  })
}
...
function startServer() {
  ...
  function setLocale(req, res, next) {
    ...
    i18n.configure({
      ...
      register: res,
      directory: path.join(__dirname, 'locales'), // 翻译json文件的路径
  ...
  app.get('/robots.txt', function(req, res, next) {
    const stream = fs.createReadStream(path.join(__dirname, './robots.txt'), {
      flags: 'r'
    })
    stream.pipe(res)
  })
  ...
  app.use(
    '/ueditor/ue',
    ueditor(path.join(__dirname, 'public'), qnParams, function(req, res, next) {
  ...
  app.get('/manage', authSession, function(req, res) {
    AdminResource.getAllResource(req, res).then(manageCates => {
      const adminPower = req.session.adminPower
      // console.log('adminPower', adminPower);
      const currentCates = JSON.stringify(
        siteFunc.renderNoPowerMenus(manageCates, adminPower)
      )
      if (isProd) {
  ...
  app.get('*', (req, res) => {
    ...
    <div style="text-align:center">
      ...
      <div style="font-size: 15px;color: #878d99;">
      ${res.__('label_page_404')} &nbsp;
      <a href="/">${res.__('label_backto_index')}</a></div>
    </div>
    `
    res.send(Page404)
  })
