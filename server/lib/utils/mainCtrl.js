...
const mainCtrl = {
  ...
  async getSiteInfo(req, res, next) {
    const configs = await SystemConfig.getSystemConfigs(req, res, next)
    const { siteName, siteDiscription, siteKeywords, siteAltKeywords } =
      configs[0] || []

    const { title, des, keywords } = req.query
    const pageTitle = title ? title + ' | ' + siteName : siteName
    const discription = des || siteDiscription
    const key = keywords || siteKeywords
    ...
    try {
      for (const md of modules) {
        ...
        if (md.action.indexOf('get_document') > -1) {
          ...
          pageData.currentCateList = await mainCtrl.getCategoryByContentId(
            req,
            res,
            next
          )
          ...
          const docInfo = await mainCtrl.getOneDocument(req, res, next)
          // 查询不到文档
          ...
          pageData.randomListData = await mainCtrl.getRandomDocumentList(
            req,
            res,
            next
          )
          ...
          pageData.replyMessageList = await mainCtrl.getMessageList(
            req,
            res,
            next
          )
          ...
          pageData.newMessageList = await mainCtrl.getMessageList(
            req,
            res,
            next
          )
          ...
          pageData.userNoticeList = await mainCtrl.getUserNoticeList(
            req,
            res,
            next
          )
          ...
          md.action === 'get_error_info_404' ||
          md.action === 'get_error_info_500'
        ) {
      ...
      const defaultTemp = await mainCtrl.getCurrentTempInfo(req, res, next)
      // 登录态
      ...
      // 针对分类页和内容详情页动态添加meta
      const defaultTempItems = defaultTemp.data.items
      if (!_.isEmpty(pageData.siteInfo)) {
        ...
        let ogImg =
          siteDomain +
          '/themes/' +
          defaultTemp.data.alias +
          '/images/mobile_logo2.jpeg'
        if (pageData.pageType === 'cate') {
          if (!_.isEmpty(pageData.cateInfo)) {
            const { defaultUrl, _id, contentTemp } = pageData.cateInfo
            ogUrl = siteDomain + '/' + defaultUrl + '___' + _id
            req.query.tempPage = mainCtrl.getCateOrDetailTemp(
              defaultTempItems,
              contentTemp,
              'cate'
            )
          ...
          if (!_.isEmpty(pageData.documentInfo)) {
            ogUrl =
              siteDomain + '/details/' + pageData.documentInfo._id + '.html'
            if (
              pageData.documentInfo.sImg &&
              pageData.documentInfo.sImg.indexOf('defaultImg.jpg') < 0
            ) {
              ogImg = siteDomain + pageData.documentInfo.sImg
            }
            const parentCateTemp =
              pageData.documentInfo.categories[0].contentTemp
            req.query.tempPage = mainCtrl.getCateOrDetailTemp(
              defaultTempItems,
              parentCateTemp,
              'detail'
      ...
      // 读取国际化文件信息
      const localKeys = await siteFunc.getSiteLocalKeys(res)
